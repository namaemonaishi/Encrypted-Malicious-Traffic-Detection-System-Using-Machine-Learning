...
class ConnectionTuple:
    ...
    def compute_conn_features(self, conn_log):
        ...
        self.orgp_raw = conn_log['id.orig_p']
        self.dstp_raw = conn_log['id.resp_p']
        try:
    ...
    def compute_x509_features(self, x509_log):
        ...
        CN = 0
        self._subject_is_com.append(x509_log['certificate.subject'])
        for key in subject:
            '''
            if 'CN=' in key:
                CN += 1
...
'''
        ...
        CN = 0
        self._issuer_is_com.append(x509_log['certificate.issuer'])
        for key in issuer:
            '''
            if 'CN=' in key:
                CN += 1
    ...
    def is_SNI_in_cert(self, ssl_log, x509_log):
        if '-' not in ssl_log['server_name']:
            ...
            self.SNIlist.append(ssl_log['server_name'])
            # number of domain in san in x509
            if '-' not in x509_log['san.dns']:
    ...
    def orgp(self):
        return self.orgp_raw
    
    def dstp(self):
        return self.dstp_raw
    
    def printsni(self):
        if self.SNIlist:
            return self.SNIlist[0]
        else:
            strnull=""
            return strnull
    
    def printoip(self):
        strip = "172.16.108.102"
        return strip
    
    def printdip(self):
        return "10.138.85.124"
    
    def apppr(self):
        return "tls"
    
    def eventID(self):
        return "0001014f-54e2-4b87-aa73-4bf0eb604022"
    ...
    def ssl_version(self):
        if self.ssl_version_dict:
            ...
            ssl_version.sort()
            if ssl_version[0] == "TLSV12":
                return "TLS 1.2"
            if ssl_version[0] == "TLSV13":
                return "TLS 1.3"
            if ssl_version[0] == "TLSV11":
                return "TLSv1"
            else:
                return ""
